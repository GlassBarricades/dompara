...
export function ViewHistoryProvider({ children }: { children: ReactNode }) {
    ...
    return <ViewHistoryStoreContext.Provider value={store}>{children}</ViewHistoryStoreContext.Provider>
}

export function useViewHistoryStore() {
    const store = useContext(ViewHistoryStoreContext)
    if (!store) {
        throw new Error('useViewHistoryStore must be used within ViewHistoryProvider')
    }
    return store
}
