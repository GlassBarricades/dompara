...
export function FeaturedProducts({ products }: FeaturedProductsProps) {
    ...
    const totalPages = Math.max(1, Math.ceil(safeProducts.length / Math.max(1, slidesPerView)))
    const currentPage = Math.min(totalPages, Math.floor(current / Math.max(1, slidesPerView)) + 1)
        ...
        <section className="container mx-auto px-4 py-10 space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Товары, которые чаще всего выбирают наши клиенты</p>
                </div>
                <Link href="/catalog" className="hidden text-sm text-primary underline-offset-4 hover:underline md:inline-flex">
                    Смотреть все товары
                </Link>
            </div>

            <Carousel opts={{ align: 'start', loop: false }} setApi={setCarouselApi} className="relative">
                <CarouselContent>
                                ...
                                <Link href={`/product/${product.slug}`} className="flex-1">
                                    <div className="relative h-48 overflow-hidden rounded-t-lg border-b bg-muted">
                                        {product.main_image_url ? (
                                            ...
                                            <div className="flex h-full items-center justify-center text-xs text-muted-foreground">Нет изображения</div>
                                        )}
                                    ...
                                    <div className="p-4 space-y-2">
                                        <div className="flex items-start justify-between gap-2">
                                            <h3 className="font-medium line-clamp-2 group-hover:text-primary transition-colors flex-1">{product.name}</h3>
                                        ...
                                        {product.short_description && (
                                            <div
                                                className="text-xs text-muted-foreground line-clamp-2 [&_p]:m-0"
                                        ...
                                        <div className="space-y-1 pt-2">
                                            ...
                                            <div className="flex items-center justify-between">
                                                <div className="text-lg font-semibold">{product.price.toLocaleString('ru-RU')} BYN</div>
                                            </div>
                                        </div>
                                    </div>
                                </Link>
                ...
                {mounted && safeProducts.length > slidesPerView && (
                    <div className="mt-6 flex flex-wrap items-center justify-between gap-3" suppressHydrationWarning>
                            ...
                            <div className="flex min-w-0 flex-1 gap-0.5 sm:gap-1 overflow-x-auto overscroll-contain [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
                                {Array.from({ length: totalPages }).map((_, i) => (
                                    <button
                                        key={i}
                                        type="button"
                                        onClick={() => carouselApi?.scrollTo(i * Math.max(1, slidesPerView))}
                                        className={`min-h-0 min-w-0 h-1 sm:h-2 rounded-full transition-all ${
                                            currentPage === i + 1 ? 'w-2.5 sm:w-6 bg-primary' : 'w-1 sm:w-2 bg-muted-foreground/40 hover:bg-muted-foreground/70'
                                        }`}
            ...
            <div className="text-center">
                <Link href="/catalog" className="inline-flex items-center text-sm text-primary underline-offset-4 hover:underline md:hidden">
                    Смотреть все товары
                </Link>
            </div>
        </section>
