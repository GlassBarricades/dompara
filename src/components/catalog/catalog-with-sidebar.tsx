...
import type { CatalogCategoryNode, FilterableAttributeDefinition, ProductAttributeValueRow } from '@/lib/catalog-api'
...
export function CatalogWithSidebar({
    ...
    const [selectedAttrs, setSelectedAttrs] = useState<Record<string, AttrFilterState>>(urlSelectedAttrs)
        ...
        const activeAttrFilters = Object.entries(selectedAttrs).filter(
            ([, v]) =>
                ...
                ((v.type === 'multiselect' && Array.isArray(v.value) && v.value.length) ||
                    ...
                    (v.type === 'number' && v.value && (typeof v.value.min === 'number' || typeof v.value.max === 'number')))
        )
    ...
    const activeFilterChips = useMemo(() => {
        ...
        Object.entries(selectedAttrs).forEach(([attrId, filter]) => {
            ...
            if ((filter.type === 'select' || filter.type === 'string') && Array.isArray(filter.value) && filter.value.length > 0) {
                filter.value.forEach((val) => {
                    const option = Array.isArray(attr.options) ? attr.options.find((o: any) => o.value === val) : null
                    chips.push({
                        id: `${attrId}-${val}`,
    ...
    const processed = useMemo(() => {
        ...
        let list = products.filter((p) => {
            ...
            for (const [attrId, filter] of activeAttrFilters) {
                ...
                if (filter.type === 'select' || filter.type === 'string') {
                    ...
                    const hasIntersection = selected.some((v) => rawArr.map(String).includes(String(v)))
                    if (!hasIntersection) return false
                } else if (filter.type === 'boolean') {
                    if (filter.value === 'all') continue
                    const boolVal = Boolean(raw)
                    if ((filter.value === 'true' && !boolVal) || (filter.value === 'false' && boolVal)) {
                        return false
                    }
                } else if (filter.type === 'number') {
        ...
        return list
    }, [products, search, minPrice, maxPrice, sort, selectedAttrs, attributeValues])
    ...
    const renderAttributeFilters = () => {
        ...
        return (
            <div className="space-y-3 border-t pt-3 mt-2">
                <div className="text-xs font-semibold uppercase text-muted-foreground">Характеристики</div>
                <div className="space-y-3 max-h-[260px] overflow-y-auto pr-1">
                    {filterAttributes.map((attr) => {
                        ...
                        if (attr.data_type === 'select' || attr.data_type === 'multiselect') {
                            ...
                            const selectedValues: string[] = (current?.value as string[]) ?? []

                            return (
                                <div key={attr.id} className="space-y-1">
                                    <div className="text-xs font-medium text-foreground">{attr.name}</div>
                                    <div className="space-y-1">
                                        {Array.isArray(attr.options) &&
                                            attr.options.map((opt: any) => {
                                                const value = String(opt.value)
                                                const checked = multiple ? selectedValues.includes(value) : selectedValues[0] === value
                                                return (
                                                    <label key={value} className="flex items-center gap-2 text-[11px] text-muted-foreground">
                                                        <input
                                                            type="checkbox"
                                                            ...
                                                            onChange={(e) => {
                                                                setSelectedAttrs((prev) => {
                                                                    ...
                                                                    let values: string[] =
                                                                        prevEntry && Array.isArray(prevEntry.value) ? [...(prevEntry.value as string[])] : []
                                                                    if (multiple) {
                                                                        if (e.target.checked) {
                        ...
                        if (attr.data_type === 'boolean') {
                            const val: 'all' | 'true' | 'false' = current && current.type === 'boolean' ? current.value : 'all'
                            return (
                                <div key={attr.id} className="space-y-1">
                                    <div className="text-xs font-medium text-foreground">{attr.name}</div>
                                    <select
                        ...
                        if (attr.data_type === 'string') {
                            const allValues = Array.from(
                                new Set(attributeValues.filter((v) => v.attribute_id === attr.id && v.value != null).map((v) => String(v.value)))
                            )
    ...
    return (
        <>
            ...
            <div className="md:hidden fixed bottom-4 left-4 right-4 z-30">
                <Button onClick={() => setMobileFiltersOpen(true)} className="w-full shadow-lg h-12 text-base font-medium" size="lg">
            ...
            <div className="grid gap-6 md:grid-cols-[260px,minmax(0,1fr)]">
                <aside className="hidden md:block space-y-4">
                    <CatalogTree tree={tree} activeCategorySlug={activeCategorySlug} activeSubcategorySlug={activeSubcategorySlug} />

                    <div className="space-y-3 rounded-md border bg-muted/30 p-3 text-sm">
                        <div className="flex items-center justify-between gap-2">
                            <div className="text-xs font-semibold uppercase text-muted-foreground">Фильтры</div>
                            <div className="flex items-center gap-2">
                                {activeFiltersCount > 0 && (
                                    <span className="text-xs bg-primary text-primary-foreground px-2 py-0.5 rounded-full">{activeFiltersCount}</span>
                                )}
                                <button
                                    type="button"
                        ...
                        <div className="space-y-2">
                            <div className="space-y-1">
                                <label className="text-xs font-medium text-muted-foreground">Поиск по названию</label>
                                <input
                                    type="text"
                ...
                <div className="space-y-4 pb-20 md:pb-4">
                    ...
                    {processed.length === 0 ? (
                        <p className="text-sm text-muted-foreground">По текущим фильтрам ничего не найдено. Попробуйте изменить запрос или диапазон цен.</p>
                    ) : (
                        <>
                                ...
                                {paginatedProducts.map((product, index) => (
                                    <Link
                                        ...
                                        className={`relative flex rounded-lg border bg-background p-4 transition-all duration-200 hover:bg-accent hover:shadow-md ${
                                            viewMode === 'grid' ? 'flex-col hover:scale-[1.02] animate-in fade-in slide-in-from-bottom-4' : 'flex-row gap-4'
                                        }`}
                                        style={
                                            viewMode === 'grid'
                                                ? {
                                    ...
                                    >
                                        {/* Бейджи */}
                                        <div className={`absolute z-10 flex flex-col gap-1 ${viewMode === 'grid' ? 'top-2 right-2' : 'top-2 right-2'}`}>
                                            {product.is_featured && (
                                        ...
                                        <div
                                            className={`relative flex items-center justify-center overflow-hidden rounded-md border bg-muted ${
                                                viewMode === 'grid' ? (verticalCardLayout ? 'mb-3 h-80 w-full' : 'mb-3 h-56 w-full') : 'h-32 w-32 flex-shrink-0'
                                            }`}
                                        >
                                        ...
                                        <div className={`flex-1 space-y-1 ${viewMode === 'list' ? 'min-w-0' : ''}`}>
                                            {viewMode === 'grid' && <div className="text-xs font-semibold uppercase text-muted-foreground">Товар</div>}
                                            <div className={`font-medium ${viewMode === 'list' ? 'text-lg' : ''}`}>{product.name}</div>
                                            {viewMode === 'list' && product.short_description && (
                                                <p className="text-sm text-muted-foreground line-clamp-2">{product.short_description.replace(/<[^>]*>/g, '')}</p>
                                            )}
            ...
            <QuickViewModal productSlug={quickViewSlug} isOpen={quickViewSlug !== null} onClose={() => setQuickViewSlug(null)} />
