...
export const ProductPageInner = observer(({ product, attributes, similarProducts = [], reviews = [] }: ProductPageInnerProps) => {
                            ...
                            <div key={review.id} className="space-y-2 border-b pb-4 last:border-0">
                                <div className="flex items-center gap-3">
                                    {review.customer_photo_url ? (
                                        <img src={review.customer_photo_url} alt={review.customer_name} className="h-10 w-10 rounded-full" />
                                    ) : (
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="flex items-center gap-1">
                                            {Array.from({ length: 5 }).map((_, i) => (
                                                <span key={i} className={`text-sm ${i < review.rating ? 'text-yellow-500' : 'text-muted-foreground'}`}>
                                                    ★
                                                </span>
                                            ))}
    ...
    return (
        <main className="container mx-auto px-4 py-8 space-y-6">
                ...
                <aside className="space-y-4 rounded-lg border bg-background p-4">
                    <div className="space-y-2">
                        <div className="overflow-hidden rounded-md border bg-muted flex w-full items-center justify-center h-[260px] sm:h-[320px] md:h-[420px]">
                            {images.length > 0 ? (
                                <img src={images[activeIndex]} alt={product.name} className="max-h-[420px] w-auto object-contain" loading="lazy" />
                            ) : (
                                <span className="text-xs text-muted-foreground">Нет изображения товара</span>
                            )}
                        </div>
                                ...
                                {images.map((src, index) => (
                                    <button
                                        ...
                                        className={`h-16 w-24 flex-shrink-0 overflow-hidden rounded-md border ${
                                            index === activeIndex ? 'ring-2 ring-primary' : 'opacity-75 hover:opacity-100'
                                        }`}
                    ...
                    <div className="space-y-3 pt-2 border-t">
                        ...
                        {!product.is_custom_order && product.stock_quantity !== null && product.stock_quantity !== undefined && (
                            <div className="space-y-1">
                                <div className="text-xs uppercase text-muted-foreground">Остаток</div>
                                <div
                        ...
                        {product.is_custom_order && (
                            <div className="space-y-1">
                                ...
                                <div className="text-sm text-muted-foreground">Товар изготавливается под заказ</div>
                            </div>
                        )}

                        <div className="flex flex-col gap-2">
                            <div className="flex flex-wrap items-center gap-2">
                                <Button onClick={handleAddToCart} className="flex-1 md:flex-none" disabled={justAdded}>
                                    {justAdded ? 'Добавлено' : 'Добавить в корзину'}
                                </Button>
                                <Button variant="outline" onClick={() => setQuickOrderOpen(true)} className="flex-1 md:flex-none">
                                    Быстрый заказ
                                </Button>
                            </div>
                            <Link href="/cart" className="text-center px-4 py-2 text-sm text-primary underline-offset-4 hover:underline">
                                Перейти в корзину
                            </Link>
                        </div>
            ...
            {/* Похожие товары */}
            {similarProducts.length > 0 && <SimilarProducts products={similarProducts} currentProductId={product.id} />}

            {/* Модальное окно быстрого заказа */}
            <QuickOrderModal productName={product.name} productPrice={product.price} isOpen={quickOrderOpen} onClose={() => setQuickOrderOpen(false)} />
        </main>
    )
})
