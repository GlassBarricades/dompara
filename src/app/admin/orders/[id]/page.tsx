...
export default function EditOrderPage() {
    ...
    async function loadProducts() {
        ...
        try {
            const { data, error } = await supabase!.from('products').select('id, name, slug, price').eq('is_active', true).order('name', { ascending: true })
    ...
    async function logOrderChange(fieldName: string | null, oldValue: any, newValue: any, comment?: string) {
        if (!canUseSupabase || !orderId) return
    ...
    async function handleSave() {
        ...
        try {
            ...
            if (oldOrder.status === 'completed' && (status !== 'completed' || JSON.stringify(oldOrder.items) !== JSON.stringify(items))) {
                if (!window.confirm('Внимание! Заявка уже была проведена. Изменение товаров или статуса может привести к расхождениям в остатках. Продолжить?')) {
                    setSaving(false)
                    return
                }
            }
            ...
            if (status === 'completed' && oldOrder.status !== 'completed') {
                if (!window.confirm('Провести заявку? Это списает товары со склада. Продолжить?')) {
                    setSaving(false)
                    return
                }
                ...
                try {
                    for (const item of items) {
                        ...
                        if (!product || product.is_custom_order) {
                            console.warn(`Товар не найден или под заказ: ${item.id} (${item.name})`)
                            continue
                        }
                        ...
                        if (quantity <= 0) {
                            console.warn(`Неверное количество для товара: ${item.id} (${item.name})`)
                            continue
                        }
                        ...
                        if (updateError) {
                            console.error(`Ошибка обновления остатка для товара ${product.id}:`, updateError)
                            throw updateError
                        }
                    ...
                    setError(`Не удалось списать остатки: ${err instanceof Error ? err.message : 'Неизвестная ошибка'}. Проверьте логи в консоли.`)
                    setSaving(false)
                    return
                }
            }

            const { error } = await supabase!.from('orders').update(newOrderData).eq('id', orderId)

            if (error) throw error

            // Логируем изменения
            if (oldOrder.customer_name !== newOrderData.customer_name) {
                await logOrderChange('customer_name', oldOrder.customer_name, newOrderData.customer_name)
            }
            ...
            if (oldOrder.status !== newOrderData.status) {
                await logOrderChange(
                    ...
                    oldOrder.status,
                    newOrderData.status,
                    `Статус изменен с "${STATUS_LABELS[oldOrder.status]}" на "${STATUS_LABELS[newOrderData.status]}"`
                )
            }
            if (JSON.stringify(oldOrder.items) !== JSON.stringify(newOrderData.items)) {
        ...
        <section className="space-y-6">
            <header className="flex items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Измените данные заявки и товары.</p>
                </div>
                <Button asChild variant="outline">
                    <Link href="/admin/orders">Назад к заявкам</Link>
                </Button>
            ...
            {order.status === 'completed' && (
                <div className="rounded-md border-2 border-emerald-600 bg-emerald-100 p-4 text-sm font-medium text-emerald-900 dark:border-emerald-500 dark:bg-emerald-950 dark:text-emerald-100">
                    <strong className="font-semibold">Внимание:</strong> Заявка уже проведена. Товары списаны со склада. Изменение товаров или статуса может привести к
                    расхождениям в остатках.
                </div>
            )}
            ...
            <div className="space-y-6">
                ...
                <div className="rounded-md border bg-background p-4 md:p-6 space-y-4">
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <div className="space-y-1">
                            ...
                            <input type="tel" className="w-full rounded-md border px-3 py-2 text-sm" value={phone} onChange={(e) => setPhone(e.target.value)} required />
                        </div>
                    ...
                    <div className="space-y-1">
                        ...
                        <textarea className="w-full rounded-md border px-3 py-2 text-sm min-h-[80px]" value={comment} onChange={(e) => setComment(e.target.value)} />
                    </div>
                                ...
                                <tbody>
                                    {items.map((item, index) => (
                                        <tr key={index} className="border-t">
                                            ...
                                            <td className="px-3 py-3 text-right font-medium">{item.price * item.quantity} BYN</td>
                                            <td className="px-3 py-3 text-right">
                                                <div className="flex justify-end gap-2">
                                                    <Button size="sm" variant="outline" onClick={() => handleEditItem(index)}>
                                                        Изменить
                                                    </Button>
                                                    <Button size="sm" variant="ghost" onClick={() => handleRemoveItem(index)} className="text-red-600 hover:text-red-700">
                                                        Удалить
                                                    </Button>
                                                </div>
                                            </td>
                                    ...
                                    <tr className="border-t bg-muted/30">
                                        ...
                                        <td className="px-3 py-3 text-right font-semibold">{totalAmount} BYN</td>
                                        <td></td>
                                    </tr>
                                </tbody>
                            </table>
                ...
                <div className="flex gap-2">
                    ...
                    <Button variant="outline" onClick={() => router.push('/admin/orders')} disabled={saving}>
                        Отмена
                    </Button>
                </div>
            </div>
            ...
            <Dialog open={editItemDialogOpen} onOpenChange={setEditItemDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>{editingItemIndex !== null ? 'Изменить товар' : 'Добавить товар'}</DialogTitle>
                        <DialogDescription>Выберите товар и укажите количество</DialogDescription>
                    </DialogHeader>
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={editingItemIndex !== null ? handleUpdateItem : handleAddItem}>{editingItemIndex !== null ? 'Сохранить' : 'Добавить'}</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </section>
